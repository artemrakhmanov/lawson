...
function buildSystem(directives: string[], brief: VoiceBrief, drift: number): string {
  const lines = [
    ...
    "- PRESERVE GRAMMATICAL PERSON AND POINT OF VIEW EXACTLY. Keep first-person content ('I', 'my') in",
    "  the first person; keep 'we'/impersonal content impersonal. Do NOT flip person in either direction,",
    "  and do NOT introduce a first-person 'I' the content does not already have. Never re-voice the text",
    "  as a lawyer addressing the reader ('I want to be straightforward with you', 'I would gently flag').",
