...
> **⚠️ Layout & seam superseded — see [`00-master-order.md`](00-master-order.md) (R1/R4) and [`lawguistics/00-overview.md`](lawguistics/00-overview.md).** The design below stands; two physical details were resolved later: (1) **everything is consolidated under `src/lib/services/lawguistics/`** — there is **no separate `services/signatures/` dir** (read every `services/signatures/…` path here as `lawguistics/…`); (2) the frozen seam is **`condition(text, target: VoiceSignature, drift, origin?)`** (4-arg, `origin` defaults to roster-median) — read the 3-arg `condition(text, target, drift)` notation throughout as that resolved signature. The per-stage build decomposition lives in `docspecs/lawguistics/`.
